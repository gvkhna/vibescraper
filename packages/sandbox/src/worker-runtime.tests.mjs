...
const securityTests = {
  ...
  shouldResolveCurrentDir: () => {
    ...
    if (!cwd || !cwd.endsWith('/')) throw new Error('CWD resolution failed')
  ...
  shouldReadWorkerMjsFile: async () => {
    ...
    if (!content.includes('sandbox')) throw new Error('Unexpected worker.mjs contents')
  ...
  shouldListFilesInDir: async () => {
    ...
    if (!files.includes('worker.mjs')) throw new Error('worker.mjs not listed')
  ...
  shouldWriteAndReadBack: async () => {
    ...
    if (content !== 'sandbox test write') throw new Error('File write/read mismatch')
