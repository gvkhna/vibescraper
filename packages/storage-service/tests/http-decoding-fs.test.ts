...
import { brotliCompressBytesSync, gzipCompressBytesSync } from '../src'
...
describe('storageService filesystem - HTTP Serving', () => {
  ...
  it('br: should handle query parameters for encoding', async () => {
    ...
    const compressedBytes = brotliCompressBytesSync(originalBytes)
  ...
  it('gzip: should handle query parameters for encoding', async () => {
    ...
    const compressedBytes = gzipCompressBytesSync(originalBytes)
