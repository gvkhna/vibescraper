...
export default {
  rules: {
    'enforce-zustand-selectors': {
      meta: {
        type: 'problem',
        docs: {
          description:
            'Ensure useStore selectors are simple state paths with no operations or logic'
        },
        schema: [],
        messages: {
          ...
          noOperations: 'useStore selectors must be simple state paths only. No operations, conditionals, or fallback values allowed. Move all logic to component body.',
        }
      },
      create(context) {
        return {
          CallExpression(node) {
            ...
            if (!isSimpleStateAccess(selector.body)) {
              context.report({
                node: selector,
                messageId: 'noOperations'
              })
            ...
            function isSimpleStateAccess(node) {
              switch (node.type) {
                case 'MemberExpression':
                  ...
                  return isSimpleStateAccess(node.object)
                ...
                case 'ChainExpression':
                  ...
                  return isSimpleStateAccess(node.expression)
