...
export function StatusBadge({ status = "ready" }: { status?: "ready" | "active" | "paused" | "error" | "processing" }) {
  const map = {
  ...
  const m = map[status]
  return (
    <span className={cn("inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs bg-white/5 border border-white/10", m.ring)}>
